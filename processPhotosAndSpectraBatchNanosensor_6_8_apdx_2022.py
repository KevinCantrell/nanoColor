...
riValues_20220628 = [1.0000, 1.3325, 1.3480, 1.3617, 1.3735, 1.3844, 1.3954]
...
if processSpectra:                    
    ...
    rangeBoolCIE = (dfSpectrum['Wavelength'] >= 380) & (dfSpectrum['Wavelength'] <= 780)
    ...
    spectraData=[]
    rawSpectra=[]    
    rangeBool=(dfSpectrum['Wavelength']>=480)&(dfSpectrum['Wavelength']<=650)
    ...
    for filename in filesToProcessSpectrum:
        ...
        rangeBool=(dfSpectrum['Wavelength']>=480)&(dfSpectrum['Wavelength']<=650)#This takes the x data from dfSpectrum and makes a Boolean array with values "true" between 500 and 600 and "false" everywhere else
        ...
        rangeBoolIntens=(dfSpectrum['Absorbance']>=0.75*maxVals['Absorbance'])&(dfSpectrum['Wavelength']>=480)&(dfSpectrum['Wavelength']<=650)
        ...
        popt, pcov = curve_fit(gaussian,dfSpectrum['Wavelength'][rangeBoolIntens],dfSpectrum['Absorbance'][rangeBoolIntens],p0=[520,30,87])
        ...
        sensorMedium=filename[-3:]
        slide=os.path.dirname(os.path.dirname(filename))[6:9]
        ...
        if sensorMedium  == '50%':
            ri = riValues[6]
        spectraData.append({'medium':sensorMedium,'filename': filename,'cTime': cTime.strftime("%Y-%m-%d %H:%M:%S"),'R':RGB[0],'G':RGB[1],'B':RGB[2],'H':HSV[0],'S':HSV[1],'V':HSV[2],'L*':LAB[0],'a*':LAB[1],'b*':LAB[2],'r':rgb[0],'g':rgb[1],'b':rgb[2],'gaussLambda':gaussLambda,'smoothedDataLambda':smoothedDataLambda,'derDataLambda':derDataLambda,'type':gen,'ri':ri,'slide':slide,'prep':prep})
...
#                highScale=np.max(chanMask)
    ...
    #            slide=os.path.basename(fileName)[os.path.basename(fileName).lower().find('sample')+6:os.path.basename(fileName).lower().find('sample')+7]
                ...
                slide=os.path.dirname(os.path.dirname(os.path.dirname(fileName)))[6:9]
                #last 10 characters of folder name should be the date of experiment
                prep=os.path.dirname(os.path.dirname(os.path.dirname(fileName)))[-10:]
