...
#M. N. Polyanskiy, "Refractive index database," https://refractiveindex.info. Accessed on 2019-09-30.
...
#        RGB, HSV, LAB, XYZ, rgb, rat, RGBg=absorbanceToTristim(waves,bextArray/scaleFactorPoly,Yr,gammaFlag=True)
#        lmax=waves[(waves>450) & (waves<800)][np.argmax(qext[(waves>450) & (waves<800)])]
#        colorDataPoly.append({'lmax':lmax,'ri': ri,'diameter': diameter,'R':RGB[0],'G':RGB[1],'B':RGB[2],'H':HSV[0],'S':HSV[1],'V':HSV[2],'L*':LAB[0],'a*':LAB[1],'b*':LAB[2],'r':rgb[0],'g':rgb[1],'b':rgb[2],'R/G':rat[0],'R/B':rat[1],'G/B':rat[2],'Rg':RGBg[0],'Gg':RGBg[1],'Bg':RGBg[2]})
...
#    dfColorPoly=dfColorPoly[['ri','diameter','R','G','B','H','S','V','L*','a*','b*','r','g','b','R/G','R/B','G/B','Rg','Gg','Bg','lmax']]
